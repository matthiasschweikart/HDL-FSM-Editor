...
class CanvasWindow:
    ...
    def __init__(self, window_x, window_y, tags, padding, entry_dicts, additional_move_func) -> None:
        self.difference_x = 0
        self.difference_y = 0
        self.borderwidth = 0
        ...
        self._add_bindings_to_frame()
        self.label_ids = []
        self.text_ids = []
        for entry_dict in entry_dicts:
            self._add_label_and_text_widget_to_frame(entry_dict)
        self._add_bindings_to_labels_and_text_widgets()
    ...
    def _add_bindings_to_frame(self):
    ...
    def _add_bindings_to_labels_and_text_widgets(self):
        for label_id in self.label_ids:
            self.funcids_label_enter.append(label_id.bind("<Enter>", lambda event: self._start_editing()))
            label_id.bind(
                "<Button-1>",
                lambda event, label_id=label_id: move_handling_canvas_window.MoveHandlingCanvasWindow(
                    event, label_id, self.window_id
                ),
            )
        for i, text_id in enumerate(self.text_ids):
            self.funcids_text_enter.append(text_id.bind("<Enter>", lambda event: self._start_editing()))
        ...
        for canvas_id in self.label_ids + self.text_ids:
            for seq in seq1_list:
                canvas_id.bind(seq, lambda event, custom_text_id=canvas_id: self._zoom_by_wheel(event, custom_text_id))
    ...
    def _start_editing(self) -> None:
        ...
        self.funcid_canvas_enter = project_manager.canvas.bind("<Motion>", lambda event: self._stop_editing(), "+")
        for i, text_id in enumerate(self.text_ids):
            self.old_text[i] = text_id.get("1.0", tk.END)
        self._show_window_as_selected()
        self._prepare_deletion_by_delete_key()
    ...
    def _restore_original_bindings(self) -> None:
        project_manager.canvas.bind_all("<Delete>", lambda event: canvas_delete.CanvasDelete())
        self.funcid_frame_enter = self.frame_id.bind("<Enter>", lambda event: self._start_editing())
        for i, label_id in enumerate(self.label_ids):
            self.funcids_label_enter[i] = label_id.bind("<Enter>", lambda event: self._start_editing())
        for i, text_id in enumerate(self.text_ids):
            self.funcids_text_enter[i] = text_id.bind("<Enter>", lambda event: self._start_editing())
    ...
    def _prepare_deletion_by_delete_key(self) -> None:
        ...
        canvas_delete.CanvasDelete.canvas_x_coordinate = window_canvas_coords[0] + project_manager.state_radius
        canvas_delete.CanvasDelete.canvas_y_coordinate = window_canvas_coords[1]
