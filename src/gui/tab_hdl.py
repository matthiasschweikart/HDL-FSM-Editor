...
class TabHDL:
    ...
    def _cursor_move_hdl_tab(self, *_) -> None:
        ...
        index_string = self.hdl_frame_text.index(f"@{delta_x},{delta_y}")  # index_string has the format "1.34"
        ...
        if line_number != self._line_number_under_pointer_hdl_tab:
            self.hdl_frame_text.tag_delete("underline")  # remove previous underline
            ...
            while self.hdl_frame_text.get(f"{line_number}.{start_index - 1}") == " ":
