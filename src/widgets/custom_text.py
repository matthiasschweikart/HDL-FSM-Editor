...
PROCESS_RE = re.compile(r"process(\s|\().*?\sbegin(\s|\s.*?\s)end\s+process(\s*;|\s.*?;)", re.IGNORECASE | re.DOTALL)
...
NOT_ASSIGNMENTS_RE = re.compile(r"(?:(?<=^)|(?<=;))(?![^;]*=)[^;]*;", re.IGNORECASE)
# NOT_ASSIGNMENTS_RE explanation:
...
class CustomText(CodeEditor):
    ...
    def _remove_loop_indices(self, text):
        ...
        match_objects = re.finditer(LOOP_INDEX_RE, text)
        for match_object in match_objects:
    ...
    def _split_in_lists_of_text_and_processes(self, text):
        proc_list = []
        text_list = []
        match_objects = re.finditer(PROCESS_RE, text)
        text_start_index = 0
        for match_object in match_objects:
    ...
    def _remove_variable_declarations(self, process):
        all_variable_names = []
        match_objects = re.finditer(VARIABLE_DECL_RE, process)
        for match_object in match_objects:
    ...
    def _add_read_variables_from_procedure_calls_to_read_variables_of_all_windows(self, text) -> str:
        if project_manager.language.get() != "VHDL":
            return text
        all_procedure_calls = []
        match_objects = re.finditer(NOT_ASSIGNMENTS_RE, text)
        for match_object in match_objects:
    ...
    def _add_read_variables_from_with_select_blocks_to_read_variables_of_all_windows(self, text) -> str:
        if project_manager.language.get() == "VHDL":
            all_with_selects = []
            match_objects = re.finditer(WITH_SELECT_RE, text)
            for match_object in match_objects:
    ...
    def _add_read_constants_from_case_when_to_read_variables_of_all_windows(self, text) -> str:
        if project_manager.language.get() == "VHDL":
            match_objects = re.finditer(WHEN_RE, text)
            for match_object in match_objects:
                if match_object.group(1) != "others":
                    CustomText.read_variables_of_all_windows[self] += [match_object.group(1)]
                text = text[: match_object.start()] + " " * len(match_object.group(0)) + text[match_object.end() :]
        return text
    ...
    def _add_read_variables_from_assignments_to_read_variables_of_all_windows(self, text) -> str:
        match_objects = re.finditer(ASSIGN_RHS_RE, text)  # Collect all right hand sides.
        for match_object in match_objects:
    ...
    def _add_read_variables_from_always_statements_to_read_variables_of_all_windows(self, text) -> str:
        match_objects = re.finditer(ALWAYS_RE, text)
        for match_object in match_objects:
